...
public class JwtTokenProvider {
    ...
    public String createToken(String userId, String roles, JwtTokenType tokenType) {
        ...
        } else if (tokenType == JwtTokenType.REFRESH_TOKEN) { // Refresh Token 이라면
    ...
    public TokenDTO getTokenInfo(String token) {
        ...
        // TokenDTO는 java17의 Record 객체 사용했기에 빌더패턴 적용함
        TokenDTO rDTO = TokenDTO.builder()
                .userId(userId)
                .role(role)
                .build();
    ...
    /**
     *  쿠키 및 인증해더(Bearer) 저장된 JWT 토큰(Access Token, Refresh Token) 가져오기
     *
    ...
    public String resolveToken(HttpServletRequest request, JwtTokenType tokenType) {
        ...
        // Cookies 에 토큰이 존재하지 않으면, Bearer 토큰에 값이 있는지 확인함
        if (token.length() == 0) {
